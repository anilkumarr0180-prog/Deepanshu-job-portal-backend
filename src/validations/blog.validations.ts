...
const candidateStatusSchema = z.preprocess(
  (val) => (typeof val === "string" ? val.toLowerCase() : val),
  z.enum([BLOG_STATUS.DRAFT, BLOG_STATUS.PUBLISHED])
);

export const createCandidateBlogSchema = z.object({
  body: z.object({
    title: z
      .string()
      .trim()
...
export const updateCandidateBlogSchema = z.object({
  params: z.object({
    id: z.string().regex(objectIdRegex, "Invalid blog ID format."),
  }),
  body: z
...
export const getCandidateBlogsQuerySchema = z.object({
  query: z.object({
    page: z.string().regex(/^\d+$/, "Page must be a positive integer.").optional(),
    limit: z.string().regex(/^\d+$/, "Limit must be a positive integer.").optional(),
